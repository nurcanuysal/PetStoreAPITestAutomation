...
namespace PetStoreTest.FunctionsTest.OrderFunctionsTest
{
	public class OrderFunctionsTests
	{
        ...
        public void VerifyPetInventoryOrderStatus()
        {
            ...
            var order = OrderHelpers.PrepareOrder((int)pet.Id, DateTime.Now.ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss.fff+00:00"), OrderStatus.delivered, 1, true);
        ...
        public void OrderCRUDTest()
        {
            ...
            var orderCreated = OrderHelpers.PrepareOrder((int)pet.Id, DateTime.Now.ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss.fff+00:00"), OrderStatus.placed, 1, true);
            ...
            var getResponse = orderFunctions.FindPurchaseOrderById((int)newAddedOrder.Id);
            var newAddedOrderData = responseOrder.GetResponseContent<Pet>(getResponse);

            //Assert
            newAddedOrderData.Id.Should().Be(orderCreated.Id);

            orderFunctions.DeletePurchaseOrderById((int)orderCreated.Id, HttpStatusCode.OK);
        }
    }
}
